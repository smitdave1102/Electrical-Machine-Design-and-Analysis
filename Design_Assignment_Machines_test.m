...
f_elec = 50 ; % Electrical frequency [Hz]
q = 2 ; % Number of slots per pole per phase
r=2;
% Omega_mech = (800 * pi) /6;
omega_elec=((2*pi)/60)*Omega_base; 
alpha = 360/(2 * q * 3);
k_w1 = 0.933; % Electrical angular frequency [rad/s]
...
l_coil_calc =  2* 0.8* L_stack;
R_phase =(rho_Cu* l_coil_calc *  N_turn / (pi/4)*(D_wire)^2 )  * (r * (N_pole/2) /2 * q)/ (N_strand * (N_parallel^2)) ;  % Resistance per phase [ohm]


F_PM = 902285 * t_PM;
phi_r = 1.1824 * (L_stack * w_PM); %(Bmag * Area of Magnet)
Re_mag = F_PM./phi_r;
% Re_gap = MMF_gap./Phi_gap;
Re_stator_tooth = MMF_stator_tooth/Phi_gap;
Re_stator_yoke = MMF_stator_yoke/Phi_gap;
Re_rotor_yoke = MMF_rotor/Phi_gap;
Phi_noload = 1.877 * mm ;
MMF_noload = 2049; 

Re_d = (MMF_noload./Phi_noload) + Re_mag ;
Re_q = MMF_noload./Phi_noload;
alpha = 360/(2 * q * 3);
k_w = (1 + cos(alpha/2));
N = N_turn* k_w * q * r ; % Nd = Nq = N = Nturn* Kw * q* r

L_d = ((N*N)/Re_d )*(N_pole./(2*N_parallel));   
L_q = ((N*N)/Re_q )*(N_pole./(2*N_parallel));
...
Volume_statoryoke = (OD_stator - ID_stator)^2 / 4 * pi * L_stack - A_tooth_slot * L_stack * N_slot;

Volume_rotor = (OD_rotor - ID_rotor)^2 / 4 * pi * L_stack - w_mag * t_mag * L_stack * 16;


% d and q Flux Linkages at Load Condition
Flux_Linkage_PM = (Phi_noload* N* r*N_pole)/(2*N_parallel);
psy_d = (L_d * I_d) + Flux_Linkage_PM; 
psy_q = L_q * I_q;


Tem = 3/2 * (N_pole/2) * Flux_Linkage_PM * I_q + 3/2 * (N_pole/2) * (L_d-L_q)*I_d*I_q;
...
P_iron_statoryoke = (K_h * B_yoke_no_load^2 * Omega / (2*pi) + K_c * (B_yoke_no_load * Omega / (2*pi))^2) * Volume_statoryoke;

P_shaft = Tem * Omega_mech; %-Pfriction
P_input = P_shaft  + P_iron_tooth + P_iron_statoryoke + P_cu;
S = (3/2)*U_s*I;
efficiency = P_shaft/P_input;
